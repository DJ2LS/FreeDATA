...
class DataFrameFactory:
    ...
    def __init__(self, ctx):
        ...
        self.mygrid = maidenhead.generate_full_maidenhead(
    ...
    def deconstruct(self, frame, mode_name=None):
        ...
        if mode_name in ["SIGNALLING_ACK"] and int.from_bytes(frame[:1], "big") not in [
        ...
        for key, item_length in frame_template.items():
            ...
            if key in ["origin", "destination"]:
                ...
                if frametype in [
                    ...
                    FR_TYPE.ARQ_SESSION_INFO_ACK.value,
                    FR_TYPE.ARQ_BURST_ACK.value,
                ]:
    ...
    def build_arq_session_info(
        self, session_id: int, total_length: int, total_crc: bytes, snr, type
    ):
        flag = 0b00000000
    ...
    def build_arq_session_info_ack(
        self,
        session_id,
        offset,
        snr,
    ...
    def build_arq_burst_frame(
        self,
        freedv_mode: codec2.FREEDV_MODE,
        session_id: int,
        offset: int,
        data: bytes,
        speed_level: int,
    ...
    def build_p2p_connection_heartbeat(
        self, session_id, flag_has_data=False, flag_announce_arq=False
    ):
        flag = 0b00000000
        if flag_has_data:
    ...
    def build_p2p_connection_heartbeat_ack(
        self, session_id, flag_has_data=False, flag_announce_arq=False
    ):
        flag = 0b00000000
        if flag_has_data:
    ...
    def build_p2p_connection_payload(
        self,
        freedv_mode: codec2.FREEDV_MODE,
        session_id: int,
        sequence_id: int,
