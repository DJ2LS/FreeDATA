...
import { deleteBroadcastDomainFromDB } from "../js/broadcastsHandler";

// NEW: Broadcast modal component (the split-out modal)
import BroadcastModal from "./broadcast_modal.vue";
...
// Delete chat/domain
function deleteChat() { deleteCallsignFromDB(chat.selectedCallsign); }
function deleteDomain() { deleteBroadcastDomainFromDB(broadcast.selectedDomain); }
...
const beaconHistogramOptions = {
  ...
  scales: {
    x: { position: 'bottom', display: true, min: -10, max: 15, ticks: { display: false }, text: 'timestamp' },
    y: { display: true, min: -15, max: 15, ticks: { display: true }, text: 'SNR' },
  },
};
...
<template>
  ...
  <!-- Station Info Modal -->
  <div id="dxStationInfoModal" ref="modalElement" class="modal fade" tabindex="-1" aria-hidden="true">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="alert alert-primary" role="alert">
            ...
            direction, FreeDATA is going somewhen. For now you can save only your personal data,
            so we can optimize and improve the database. In future this data can be requested by a remote station.
          </div>
          <ul>
  ...
  <div id="updaterReleaseNotes" ref="modalElement" class="modal fade" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <span id="updater_last_version" class="input-group-text"/>
          <span id="updater_last_update" class="input-group-text ms-1"/>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"/>
        </div>
        <div class="modal-body">
          <div class="modal-dialog modal-dialog-scrollable">
            <div id="updater_release_notes" class=""/>
          </div>
        </div>
        <div class="modal-footer">
  ...
  <!-- Delete chat modal -->
  <div id="deleteChatModal" ref="modalElement" class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 id="deleteChatModalLabel" class="modal-title fs-5">
        ...
        <div class="modal-body">
          <div class="card">
            ...
            <div class="card-body">
              <Bar :data="beaconHistogramData" :options="beaconHistogramOptions" width="300" height="100" />
            </div>
          </div>
          ...
          <div class="card mt-3">
            ...
            <div class="card-body">
              <button type="button" class="btn btn-danger" data-bs-dismiss="modal" @click="deleteChat">
                {{ $t('modals.deletechat') }}
              </button>
            </div>
          </div>
  ...
  <!-- Delete broadcast domain modal -->
  <div id="deleteBroadcastModal" ref="deleteBroadcastModalElement" class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 id="deleteBroadcastModalLabel" class="modal-title fs-5">
            {{ chat.selectedCallsign }} {{ $t('modals.options') }}
          </h1>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"/>
        </div>
        <div class="modal-body">
          <div class="card mt-3">
            ...
            <div class="card-body">
              <button type="button" class="btn btn-danger" data-bs-dismiss="modal" @click="deleteDomain">
  ...
  <!-- Message Info Modal -->
  <div id="messageInfoModal" ref="modalElement" class="modal fade" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h1 id="messageInfoModalLabel" class="modal-title fs-5">
        ...
        <div class="modal-body">
          ...
          <div class="card mt-2">
            ...
            <div class="card-body">
              <Line :data="transmissionSpeedChartDataMessageInfo" :options="transmissionSpeedChartOptionsMessageInfo" />
            </div>
          </div>
        </div>
...
<div
  ...
  <!-- AUDIO MODAL -->
  <div id="audioModal" class="modal fade" data-bs-backdrop="static" tabindex="-1">
    <div class="modal-dialog modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
          ...
          <div class="input-group input-group-sm mb-1">
            ...
            <button id="sendTestFrame" type="button" class="btn btn-danger" @click="sendModemTestFrame()">
          ...
          <div class="input-group input-group-sm mb-1">
            ...
            <span class="input-group-text w-75">
              <input id="audioLevelRX" v-model.number="settings.remote.AUDIO.rx_audio_level" type="range" class="form-range" min="-30" max="20" step="1" @change="onChange">
            </span>
          </div>
          <div class="input-group input-group-sm mb-1">
            ...
            <span class="input-group-text w-75">
              <input id="audioLevelTX" v-model.number="settings.remote.AUDIO.tx_audio_level" type="range" class="form-range" min="-30" max="20" step="1" @change="onChange">
            </span>
          </div>

          <div class="input-group input-group-sm mb-1">
            <span class="input-group-text">{{ $t('modals.audiotuningtransmitsine') }}</span>
            <button id="sendTestFrame" type="button" class="btn btn-success" @click="sendSineTone(true)">
              {{ $t('modals.audiotuningtransmitsine30s') }}
            </button>
            <button id="sendTestFrame" type="button" class="btn btn-danger" @click="sendSineTone(false)">
              {{ $t('general.stop') }}
            </button>
          </div>
        </div>
  ...
  <!-- STATION INFO MODAL -->
  <div id="stationInfoModal" class="modal fade" tabindex="-1" aria-labelledby="stationInfoModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="alert alert-primary" role="alert">
            <strong> Please note:</strong> This is a preview to show you the direction, FreeDATA is going somewhen.
            For now you can save only your personal data, so we can optimize and improve the database.
            In future this data can be requested by a remote station.
          </div>
          <div class="input-group mb-1">
            <span class="input-group-text"><i class="bi bi-person-fill" /></span>
            <input v-model="station.stationInfo.info.name" type="text" class="form-control" placeholder="Name">
          </div>
          <div class="input-group mb-1">
            <span class="input-group-text"><i class="bi bi-geo-alt-fill" /></span>
            <input v-model="station.stationInfo.info.city" type="text" class="form-control" placeholder="City">
          </div>
          <div class="input-group mb-3">
            <span class="input-group-text"><i class="bi bi-person-fill" /></span>
            <input v-model="station.stationInfo.info.age" type="text" class="form-control" placeholder="Age">
          </div>
          <div class="input-group mb-1">
            <span class="input-group-text"><i class="bi bi-broadcast-pin" /></span>
            <input v-model="station.stationInfo.info.radio" type="text" class="form-control" placeholder="Radio">
          </div>
          <div class="input-group mb-3">
            <span class="input-group-text"><i class="bi bi-cone-striped" /></span>
            <input v-model="station.stationInfo.info.antenna" type="text" class="form-control" placeholder="Antenna">
          </div>
          <div class="input-group mb-1">
            <span class="input-group-text"><i class="bi bi-globe" /></span>
            <input v-model="station.stationInfo.info.website" type="url" class="form-control" placeholder="Website">
          </div>
          <div class="input-group mb-3">
            <span class="input-group-text"><i class="bi bi-envelope-fill" /></span>
            <input v-model="station.stationInfo.info.email" type="email" class="form-control" placeholder="Email">
          </div>

          <div class="mb-3">
            <div v-for="(url, platform) in station.stationInfo.info.socialMedia" :key="platform">
              <div class="input-group mb-1">
                <span class="input-group-text"><i :class="`bi bi-${platform}`" /></span>
                <input v-model="station.stationInfo.info.socialMedia[platform]" type="url" class="form-control" :placeholder="`${platform.charAt(0).toUpperCase() + platform.slice(1)} URL`">
              </div>
            </div>
          </div>

          <div class="mb-3">
            <label class="input-group-text" for="comments"><i class="bi bi-textarea-resize" /> Comments</label>
            <textarea v-model="station.stationInfo.info.comments" class="form-control" rows="3"/>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
          ...
          <button type="button" class="btn btn-primary" @click="updateStationInfo">
