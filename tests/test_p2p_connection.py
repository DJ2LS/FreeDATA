...
class TestP2PConnectionSession(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.iss_socket_interface_manager = SocketInterfaceHandler(
            cls.iss_modem,
            config_manager,
            StateManager(queue.Queue()),
        ...
        config_manager.write(conf)
        cls.irs_socket_interface_manager = SocketInterfaceHandler(
            cls.irs_modem,
            config_manager,
            StateManager(queue.Queue()),
            EventManager([queue.Queue()]),
        ...
        # Channels
        cls.iss_frame_dispatcher = DISPATCHER(
            cls.config,
            EventManager([queue.Queue()]),
            StateManager(queue.Queue()),
            cls.iss_modem,
            cls.iss_socket_interface_manager,
        )
        cls.irs_frame_dispatcher = DISPATCHER(
            cls.config,
            EventManager([queue.Queue()]),
            StateManager(queue.Queue()),
            cls.irs_modem,
            cls.irs_socket_interface_manager,
        )
        ...
        threading.Thread(
            target=cls.read_from_socket, args=(cls.iss_socket_cmd_client, "CMD", "ISS"), daemon=True
        ).start()
        threading.Thread(
            target=cls.read_from_socket,
            args=(cls.iss_socket_data_client, "DATA", "ISS"),
            daemon=True,
        ).start()
        threading.Thread(
            target=cls.read_from_socket, args=(cls.irs_socket_cmd_client, "CMD", "IRS"), daemon=True
        ).start()
        threading.Thread(
            target=cls.read_from_socket,
            args=(cls.irs_socket_data_client, "DATA", "IRS"),
            daemon=True,
        ).start()
    ...
    def DisabledtestConnect(self):
        ...
        self.iss_to_irs_channel = threading.Thread(
            target=self.channelWorker,
            args=(self.iss_modem.data_queue_received, self.irs_frame_dispatcher),
        )
        self.iss_to_irs_channel.start()

        self.irs_to_iss_channel = threading.Thread(
            target=self.channelWorker,
